...
podman build --platform linux/amd64 -t $REGISTRY/$NAMESPACE/$IMAGE:$NEXT_TAG ./src/ledger/ledgerwriter/

podman push $REGISTRY/$NAMESPACE/$IMAGE:$NEXT_TAG --tls-verify=false

oc set image deployment/ledgerwriter ledgerwriter=image-registry.openshift-image-registry.svc:5000/$NAMESPACE/$IMAGE:$NEXT_TAG -n $NAMESPACE

oc rollout status deployment/ledgerwriter -n $NAMESPACE
